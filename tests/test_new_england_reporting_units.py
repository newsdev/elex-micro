...
class TestConnecticutRollups(unittest.TestCase):
    """
    ...
    def test_ct_has_all_counties(self):
        """
        If CT shows 169 'counties' it's because townships are getting put up in there.
        """
        ct_counties = set()
        raw_counties = [
    ...
    def test_ct_counties_match_townships(self):
        ...
        for county in ct_counties:
            races = set([
                r['raceid'] for r in self.candidate_reporting_units if
                r['statepostal'] == "CT" and
            ...
            for race in races:
                townships = [
                    r['precinctstotal'] for r in self.candidate_reporting_units if
                    r['statepostal'] == "CT" and
                    r['level'] == "township" and
...
class TestRhodeIslandEdgeCaseReportingUnits(unittest.TestCase):
    """
    ...
    def test_existence_of_mail_ballots(self):
        ...
        for z in ri_results:
            try:
                if u"C.D." in z['reportingunitname']:
                    mail_ballots.append(z)
            except TypeError:
                pass
        self.assertTrue(len(cd_rus) > 0)
